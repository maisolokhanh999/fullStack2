...
export default function InvoiceDetailsPanel() {
  ...
  return <div className="admin-lookup-panel"><form className="admin-action-row" onSubmit={lookup}><select value={id} onChange={(event) => setId(event.target.value)} required><option value="">Chọn mã hóa đơn</option>{items.map((item) => <option key={item._id} value={item._id}>{item._id} - {item.payerName || 'Không có tên'}</option>)}</select><button type="submit" className="admin-btn">Tra cứu</button></form>{lookupError && <p className="admin-row-error">{lookupError}</p>}{invoice && <dl className="admin-detail-list">{[['Mã hóa đơn', invoice._id], ['Người thanh toán', invoice.payerName], ['Điện thoại', invoice.phoneNumber], ['Tổng tiền', invoice.totalAmount], ['Tiền cọc', invoice.depositAmount], ['Còn phải trả', invoice.finalAmount], ['Phương thức', invoice.paymentMethod], ['Trạng thái', invoice.status]].map(([label, value]) => <div key={label}><dt>{label}</dt><dd>{value ?? '—'}</dd></div>)}</dl>}</div>
}
